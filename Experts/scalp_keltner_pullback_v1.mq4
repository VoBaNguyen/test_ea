...
//---
   ...
   if(totalPos < maxPos) {
      ...
      if(isBuy || isSell) {
         // Manage orders
         if(isBuy) {
            int orderID = OrderSend(NULL,OP_BUY,lotSize,Ask,slippage,lowKN,lowKN, "Buy MA", magicNumber);
            if(orderID < 0) Alert("Order rejected. Order error: " + ErrorDescription(GetLastError()));
         } else if(isSell) {
            int orderID = OrderSend(NULL,OP_SELL,lotSize,Bid,slippage,upKN,upKN, "Sell MA", magicNumber);
            if(orderID < 0) Alert("order rejected. Order error: " + ErrorDescription(GetLastError()));
         }
      }
   }
