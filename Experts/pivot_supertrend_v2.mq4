...
void OnTick()
  {
   ...
   for(int i=0; i<ArraySize(MA200); i++) {
      MA200[i] = iMA(Symbol(), TIME_FRAME, 200, 0, MODE_SMA, PRICE_CLOSE, i+1);
   }
   ...
   bool isMA50Upward = idcUpward(MA50);
   bool isMA50Downward = idcDownward(MA50);
   bool isMA200Upward = idcUpward(MA200);
   bool isMA200Downward = idcDownward(MA200);
   bool isBuy = false;
   bool isSell = false;
   ...
   // NEW SIGNAL
   if(totalPos < maxPos) {
      if(isDowntrend && isMA50Downward && isMA200Downward) {
         isSell = true;
      } else if(isUptrend && isMA50Upward && isMA200Upward) {
         isBuy = true;
      }
   }
