...
//Enumerative for the market trend
enum ENUM_MARKET_TREND{
   UPTREND=0,                 //STOP LOSS PASSED IN POINTS
   DOWNTREND=1,               //STOP LOSS PASSED BY PRICE
   SIDEWAY=-1,               //STOP LOSS PASSED BY PRICE
};
...
input int MaxPos=20;                                                 //Maximum Positions
input int MaxLoss=300;                                               //Maximum Loss went exceed orders
input int MinLoss=50;                                                //Minimum Loss went exceed orders
...
input int Slippage=300;                                              //Slippage in points
...
void EvaluateEntry(){
   ...
   if(TotalOpenOrders>MaxPos) return; //If there are already open orders and you don't want to open more
   ...
   double MA1 = iMA(Symbol(), PERIOD_CURRENT, 20, 0, MODE_SMA, PRICE_CLOSE, 1);
   double MA2 = iMA(Symbol(), PERIOD_CURRENT, 50, 0, MODE_SMA, PRICE_CLOSE, 1);
   ENUM_MARKET_TREND marketTrend = MarketTrend(20, 50);
   ...
   if(Close[0]>BBUp && marketTrend == UPTREND && MA1 > MA2) SignalEntry=SIGNAL_ENTRY_BUY;
   ...
   if(Close[0]<BBLow && marketTrend == DOWNTREND && MA1 < MA2) SignalEntry=SIGNAL_ENTRY_SELL;
   //This is where you should insert your Entry Signal for SELL orders
   //Include a condition to open a sell order, the condition will have to set SignalEntry=SIGNAL_ENTRY_SELL
...
void ExecuteEntry(){
   ...
   if(SignalEntry==SIGNAL_ENTRY_BUY){
      ...
      Alert("Send buy order");
      SendOrder(Operation,Symbol(),OpenPrice,0,0);
   }
...
void EvaluateExit(){
   ...
   if(OrdersTotal() > 0 && !IsTradedThisBar) {
      ...
      double delta = account.info.EQUITY - account.info.BALANCE;
      double minLoss = -50.0;
      if(delta > MaxLoss) {
         SignalExit=SIGNAL_EXIT_ALL;
      } else if(delta < -MaxLoss) {
         SignalExit=SIGNAL_EXIT_ALL;
      }
   }
}
...
void SendOrder(int Command, string Instrument, double OpenPrice, double SLPrice, double TPPrice, datetime Expiration=0){
   ...
   for(int i=1; i<=OrderOpRetry; i++){
      ...
      if(TotalOpenOrders > 0) {
         LotSize = TotalOpenOrders * MinLotSize;
      }
...
ENUM_MARKET_TREND MarketTrend(int MAPeriod1, int MAperiod2) {
	double MA1[6], MA2[6];
	int arrSize = ArraySize(MA1);
	for(int i=0; i<arrSize; i++) {
		MA1[i] = iMA(Symbol(),PERIOD_CURRENT,MAPeriod1,0,MODE_EMA,PRICE_CLOSE,i+1);
		MA2[i] = iMA(Symbol(),PERIOD_CURRENT,MAperiod2,0,MODE_EMA,PRICE_CLOSE,i+1);
	}

	bool MA1Upward = idcUpward(MA1);
	bool MA1Downward = idcDownward(MA1);
	bool MA2Upward = idcUpward(MA2);
	bool MA2Downward = idcDownward(MA2);

	if(MA1Upward && MA2Upward) {
		return UPTREND;
	} else if(MA1Downward && MA2Downward) {
		return DOWNTREND;
	} else {
		return SIDEWAY;
	}
