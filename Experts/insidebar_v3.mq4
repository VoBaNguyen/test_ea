...
//---
   ...
   if(totalPos < maxPos) {
      ...
      grpInfo[0] = "undefined";
      grpInfo[1] = "undefined";
      classifyGroupCandle(1, grpInfo, TIME_FRAME);
      if(grpInfo[0] == "insidebar" && grpInfo[1] == "sell") {
         bool isExtrama = isLocalExtremum(1, "high", TIME_FRAME, 0.2);
         if(isExtrama) isSell = true;
      }
      else if(grpInfo[0] == "insidebar" && grpInfo[1] == "buy") {
         bool isExtrama = isLocalExtremum(1, "low", TIME_FRAME, 0.2);
         if(isExtrama) isBuy = true;
      }
      ...
      if(isBuy || isSell) {
         ...
         if(isBuy) {
            ...
            double SL = Ask - ATRCurr*atrMultiplier;
